...
$newStudentBalance = $stuBalance + $payment;
	...
	echo $dupliCount;
		...
		public function advancePaymentEntry(){
			if(isset($_POST['advancePay'])):
				...
				$stuAcc = $this->accmodone->studentAccountBalance( $proStu );

				// echo "<pre> this is student information";
				// print_r($stuAcc);

				// make new balance
				$newBalance = $stuAcc->balance + $totalAmount;
				...
				//$stu_acc_up = 1;
				$stu_acc_up = $this->db->where("stu_id",$proStu)->update("student_account",$updateData);
				
				// if update student account information then
				// store data in advance history table
				...
				if($stu_acc_up):
					...
					$subAmount = explode(",",implode(",",array_filter($finamount)));
				
					$branchQuery = array();

					for($i = 0;$i < count($feeidck);$i++):
						
						$duplicateCheck = $this->accmodone->duplicatAdvanceCheck( $shift,$classid,$sections,$stuclroll,$fromMonth[$i],$toMonth[$i],$year,$feeidck[$i] );
						...
						if(!$duplicateCheck):
							...
							$advanceData = array(
									...
									"sectionid"  => $sections,
									"roll" 		 => $stuclroll,
									"trans_catg" => $feeidck[$i],
									"from_month" => $fromMonth[$i],
									"to_month"	 => $toMonth[$i],
									"amount" 	 => $subAmount[$i],
									"year" 	 	 => $year,
									"comments" 	 => $comment,
							...
							array_push($branchQuery, $advanceData);
						endif;
					endfor;
					
					$advanceInsert = 0;
					if(is_array($branchQuery)):
						if(count($branchQuery)):
							$advanceInsert = $this->db->insert_batch("advancepayment",$branchQuery);
						endif;
					endif;
					...
					/**
						if insert into advanced history
						then insert into general ladger

						trans_type = 5 (advanced payment)

					**/

					if($advanceInsert):
						// get cash account balance
						$cashBalance = $this->accmodone->cashAccountBalance();

						// echo "Cash balance";
						// print_r($cashBalance);

						// make new cash balance
						$newCashBalance = $cashBalance->balance + $totalAmount;
						...
						// $updateCashAcc = 1;
						$updateCashAcc = $this->db->where("accountid",$cashBalance->accountid)->update("account_cre",$accData);

					// main ledger data
					$mainLedgerData = array(
							"id" 		   => '',
							"trans_id" 	   => $transid,
						...
						// echo "main ledger informaion";
						// print_r($mainLedgerData);
						// insert into general ledger
						$insLdg = $this->db->insert("main_ledger",$mainLedgerData);
					endif;
					redirect("account/advancePayment","location");
				endif;
			endif;
		}
