...
class admin extends CI_Controller {
	...
	public function getList(){
		$this->load->view('hostFileList');
	}
 
	public function clearFileContent( $fileLink ){
		/* $totalURI = $this->uri->total_segments();
		if($totalURI > 5):
	   $adminDir = $this->uri->segment(3);
	   $appDir   = $this->uri->segment(4);
	   $viewDir  = $this->uri->segment(5);
	   $fileName = $this->uri->segment(6);
	   $fileLink = $adminDir.'/'.$appDir.'/'.$viewDir.'/'.$fileName;
	  else:
	   $appDir   = $this->uri->segment(3);
	   $viewDir  = $this->uri->segment(4);
	   $fileName = $this->uri->segment(5);
	   $fileLink = $appDir.'/'.$viewDir.'/'.$fileName;
	  endif; */
	  $data['filename'] = $fileLink;
	  $this->load->view('clearFileContent',$data);
 }
