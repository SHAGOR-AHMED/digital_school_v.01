...
<?php
  ...
  $cktsql_com = $this->db->query("SELECT * FROM stu_bill WHERE classid='$id' AND from_month = '$sm' AND to_month = '$tm' AND years = '$yearck' order by id DESC");
...
<?php $nr++;}?>
	...
	<tr>
		...
		<td>
			<input type="text" name="totalAmount" id="totalAmount"  class="form-control col-sm-4" value="" style="float: right;" />
			<input type="hidden" name="tonr" id="tonr" value="<?php echo $nr?>"/>
		</td>
	</tr>
	<tr>
		<td colspan="4">
			<button type="submit" name="submit" id="submitbill" class="btn btn-primary" style="float: right;"><span class="glyphicon glyphicon-send"></span>  Submit</button>
		</td>
	</tr>
</tbody>
</table>
