...
// this is english in word function
...
// seven digit number
		...
		return seven_digit($n);
	}

// eight digit number
	else if($c==8){
		return eight_digit($n);
	}
...
function bnConvert($bnConvert){

// function calling of english conversion
	$enResult=$this->convertToWord($bnConvert);
	//$enResult=$bnConvert;
...
// translate english result array into bangla
	for($i=0;$i<$enCount;$i++){
		...
		if(in_array($enValue, $tens_supix)){
			...
			$i=$i+1;
			$enValue1=$enArray[$i];
			if(in_array($enValue1, $singleDigit)){  // test if this value have in tens array??
