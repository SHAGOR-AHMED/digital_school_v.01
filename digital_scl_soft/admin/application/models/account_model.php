...
// transaction id
	...
	function incInvoiceId(){
		...
		if( $invid->invoice_id ):
			...
			$invoiceKey = substr(microtime() + floor(rand()*100),0,8);
		endif;
...
// main ledger section
	...
	function inCompleteBillStd( $class,$year,$fmonth,$tmonth ){
		return $this->db->query("SELECT * FROM re_admission WHERE classid = '$class' AND syear = '$year' AND status = 1 AND stu_id NOT IN(SELECT stu_id FROM stu_bill WHERE classid = '$class' AND years = '$year'AND from_month = '$fmonth' AND to_month = '$tmonth')")->result();
	}
	...
	function getDiscount( $shift,$class,$roll,$year ){
		...
		$discount = $this->db->select("*")->from("schship")->where($data)->get()->row()->discount;
	...
	function feesCategory( $fees ){
		...
		for($i = 0;$i < count($f);$i++):
			$query = $this->db->query("SELECT c.amount,f.catg_type FROM class_fee_sett c RIGHT JOIN fee_catg f ON f.feectgid = c.feectgid WHERE c.feeid = '$f[$i]'")->row();
		...
		return substr($feeName,0,-1).'+'.substr($feeAmount,0,-1);
	}
	...
	function todayIncome(){
		...
		return $this->db->query("SELECT sum(debit) as balance FROM main_ledger WHERE trans_type IN ('2,3,5') AND date(e_date) = date('$date') ")->row()->balance;
	...
	function vanAssignReport( $data ){
		...
		return $this->db->query("SELECT v.assid,r.stu_id,rg.name,s.shift_N,stb.section_name,c.class_name,v.roll,v.edate,v.status,v.udate FROM van_assign v RIGHT JOIN re_admission r ON v.year = r.syear LEFT JOIN shift_catg s ON s.shiftid = v.shiftid LEFT JOIN class_catg c ON c.classid = v.classid LEFT JOIN section_tbl stb ON stb.sectionid = v.sectionid LEFT JOIN regis_tbl rg ON rg.stu_id = r.stu_id WHERE v.shiftid = r.shiftid AND v.classid = r.classid AND v.sectionid = r.section AND v.roll = r.roll_no AND $where")->result();
	}
...
function studentAllTransID( $studentID ){
	...
	$advance = $this->db->query("SELECT GROUP_CONCAT(trans_id) AS at FROM advancepayment WHERE stu_id = '$studentID'")->row()->at;
